...
- [x] **C6.4 HyperHead / HyperFusion 双向 OOD（OOD v2 偏离 #4，恢复条件化深度轴）** —— 训练作业 **77963/77981**(MIMIC-HH)、**77982**(MIMIC-HF)、**77983/77984**(CheXpert 两臂) 补 trial 1/2（40 run），推理作业 **78098/78099**（full-target 双向，各 15 replicate，60/60 落盘）。**H6/H7 双双否定：每条的两判据 × 两方向共八项全败**——marginal-worst 在 M→C 微正（HyperHead +0.0010 n.s. / HyperFusion +0.0016 显著）、C→M **均显著为劣**（−0.0022 / −0.0015），且四格效应量全部远小于 σ_train（|t| = 0.34–1.31）。Holm family 6→10 后既有 6 项全部保持显著、既有数字逐位不变。**深度轴（Head<Fusion<Adapt）在 M→C 上看似单调递增但整条线落在噪声带内、C→M 直接不单调 ⇒ 深度非决定因素**，与 ID 侧及实验 E 受控消融同构。⇒ **OOD v2 六臂完成，正面效应无一超过训练噪声**。报告 `docs/ood_experiment_v2_depth_arms.md`
