...
> 动机：全研究仅 HAM 一个数据集 `I(Y;A|X)>0`（⚠️ 新 OOF V-info 闸门下 HAM-age 亦退回未检出，见
> `docs/conditional_v_information_gate.md`），信号强弱与「数据集身份」完全共线，无法把
...
> 动机：唯一正信号数据集（⚠️ 按新 OOF V-info 闸门，HAM-age 已退回未检出）在 9×3 矩阵无一 p<0.05；功效不足既来自 seed 少（n=5），也来自评估端
