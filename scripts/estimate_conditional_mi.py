"""
估计 MIMIC-CXR No Finding 上的条件互信息 I(Y;A|X)，模型无关地检验「敏感属性信号是否弱」
========================================================================================
...
运行方式：
    特征抽取是对 ~199k 张图各一次前向（GPU 工作），按项目规范用 sbatch 提交，见
    slurm/estimate_conditional_mi.sh。E0–E3 分析在缓存特征上跑，随作业一并完成。
"""
...
def extract_split_features(seed: int, split: str, cfg: dict) -> dict[str, np.ndarray]:
    ...
    """
    ckpt_path = OUTPUT_DIR / f"resnet18_no_finding_seed{seed}_best_overall.pth"
...
def conditional_mi_gain(
    ...
    for mode in ("add", "int"):
        ...
        for g in range(n_g):
            ...
            per_group[str(g)] = {"delta_nll_nats": gm, "n": int(mask.sum())}
