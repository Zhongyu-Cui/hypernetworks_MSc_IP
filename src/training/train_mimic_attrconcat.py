"""
Train ResNet18-AttrConcat on MIMIC-CXR for No Finding Binary Classification
===========================================================================
属性融合变体训练流程：在 image-only baseline (train_mimic_resnet18.py) 的基础上，
**不使用超网络**，而是把离散敏感属性 (sex / race / age_group) 各自 embedding 后
broadcast 成空间图，沿通道维与图像 concat，送入 stem 加宽后的 ResNet-18
(src/models/resnet18_attrconcat.py)。backbone 的 bn1/layer1~4/avgpool/fc 与 baseline
完全一致 (ImageNet 预训练 torchvision ResNet-18)；conv1 加宽时把属性通道权重零初始化，
保证初始前向严格等价于纯预训练 backbone，因此可与 baseline 在相同预处理、相同早停设置下
直接对比公平性。

与 HyperFusion / HyperHead 训练脚本的唯一区别：模型换成 ResNet18AttrConcat（属性在输入端
拼接、从 stem 起就参与所有卷积，而非由超网络生成层参数）。其余训练策略 (AdamW + 小学习率
+ weight decay + 梯度裁剪、两种 model selection、早停监控 val Overall AUC、超参均读
configs/mimic_cxr_baseline.yaml) 与 baseline 严格保持一致。

显存：仅 conv1 输入通道由 3 增至 9，其余结构不变，显存占用与 baseline 几乎相同，
batch_size=128 在 gpus24 (24GiB) 上即可。

训练结束后按两种 model selection 策略 (best_overall / best_worstcase) 在测试集上输出
整体指标与分组公平性报告。

运行方式:
    重型任务，必须通过 sbatch 提交到 SLURM 集群 (见 slurm/train_mimic_attrconcat.sh)。
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Train ResNet18-AttrConcat on MIMIC-CXR (No Finding, attribute-conditioned)"
    )
...
def train_one_epoch(
    ...
    for i, (images, labels, sexes, races, ages) in enumerate(loader):
        # 与 baseline 不同：敏感属性进入模型，在输入端拼接为额外通道
...
def main() -> None:
    ...
    ckpt_overall_path = OUTPUT_DIR / f"attrconcat_no_finding_seed{seed}_best_overall.pth"
    ckpt_worstcase_path = OUTPUT_DIR / f"attrconcat_no_finding_seed{seed}_best_worstcase.pth"
    ...
    # pretrained=True：backbone 与 baseline 一致 (ImageNet 预训练)，conv1 属性通道零初始化
    # 保证初始前向严格等价于纯预训练 backbone
    model = ResNet18AttrConcat(num_classes=1, pretrained=cfg["model"]["pretrained"]).to(DEVICE)
    n_total, n_attr, n_backbone = model.param_breakdown()
    print(f"\nModel : ResNet18-AttrConcat (backbone=ImageNet 预训练, sex/race/age 在输入端拼接为额外通道)")
    print(f"Params: {n_total:,} (backbone {n_backbone:,} + attr-embed {n_attr:,})")
