"""
Train ResNet18 on Fitzpatrick17k for Malignant Binary Classification (Image-only)
==================================================================================
端到端 baseline 训练流程: 仅以皮肤镜 RGB 图像作为输入 (不引入 skin 肤色属性)，
预测 malignant (0=benign/non-neoplastic, 1=malignant；恶性占比约 13.5%)。

Backbone 为 ImageNet 预训练 ResNet-18 (src/models/resnet18_pretrained.py)，训练策略
与 MIMIC-CXR baseline (src/training/train_mimic_resnet18.py) 完全对齐，便于横向比较。
训练过程中同时按两种 model selection 策略保存 checkpoint：
  - best_overall  : 验证集 Overall AUC 最高 (用于早停判据)
  - best_worstcase: 验证集 Worst-case AUC (6 个肤色子群 minimax) 最高
两者训练结束后均在测试集上输出整体指标与分组公平性报告，用于对比
model selection 策略对公平性的影响。

与 MIMIC 训练脚本的关键差异:
  - Dataset 返回三元组 (image, label, skin)，单一敏感属性（肤色 I–VI → 0–5）
  - 图像是自然 RGB（preproc_224x224, uint8），transform 无需 Grayscale/Resize，
    且训练增强保留水平翻转（皮损无解剖学左右约束）
  - 公平性口径走 src/utils/fitzpatrick_fairness（6 肤色子群，无 sex/race/age 交叉）

运行方式:
    重型任务，必须通过 sbatch 提交到 SLURM 集群 (见 slurm/train_fitzpatrick_resnet18.sh)。
    通过 --seed 指定随机种子（用于多 seed 重复实验，默认取
    configs/fitzpatrick_baseline.yaml 中 training.seeds 的第一个值）。
"""
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(description="Train ResNet18 on Fitzpatrick17k (malignant, image-only)")
...
def get_dataloaders(
    ...
    (data/splits/fitzpatrick17k/{train,val,test}.csv)，此处直接加载，无需再次切分。
...
def train_one_epoch(
    ...
    for i, (images, labels, _skins) in enumerate(loader):
        # skin 不进入 image-only 模型，仍从 loader 解包以保持接口一致
...
def evaluate(model: nn.Module, loader: DataLoader, criterion: nn.Module) -> tuple[float, float, float, float]:
    ...
    （6 个肤色子群 minimax）用于「Worst-case AUC 选择」判据，两者口径与下方分组
    公平性报告一致。worst_case_auc 在无有效子群时返回 None，此处回落为 0.0（仅用于排序）。
    """
    model.eval()
    total_loss = 0.0
    correct = 0
...
def collect_predictions(model: nn.Module, loader: DataLoader) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    ...
    skin 不进入模型，仅作为分组键，用来观察 image-only 模型在不同肤色子群上的
    表现差异（隐式偏差）。
    """
    model.eval()
    all_logits, all_labels, all_skins = [], [], []
...
def main() -> None:
    ...
    # checkpoint 命名加 resample tag，避免覆盖原 baseline (resnet18_malignant_seed*) 权重
    ...
    print("\nLoading Fitzpatrick17k (malignant, image-only)...")
