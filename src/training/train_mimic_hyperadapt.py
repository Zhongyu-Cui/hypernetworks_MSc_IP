"""
Train ResNet18-HyperAdapt on MIMIC-CXR (No Finding, sex/race/age-conditioned)
=============================================================================
把 backbone **每个卷积层**（通道级乘性调制）与分类头 fc（加性低秩更新）都接上由 sex/race/age 驱动的
超网络（ResNet18HyperAdapt，初始 Δθ≈0 起步等价 baseline）。HyperAdapt 是「介入最遍布」的深层 HN
（除 stem 外每层），与 HyperHead（只改 fc）、HyperFusion（单点）构成注入深度对照。
...
运行方式：重型任务，经 sbatch 提交（见 slurm/train_mimic_hyperadapt.sh）。
"""
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(description="Train ResNet18-HyperAdapt on MIMIC-CXR (sex/race/age)")
...
def main() -> None:
    ...
    print(f"Loading MIMIC-CXR (No Finding, sex/race/age-conditioned HyperAdapt)  "
