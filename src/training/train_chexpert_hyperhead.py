"""
Train ResNet18-HyperHead on CheXpert for No Finding Binary Classification
==========================================================================
属性融合变体训练流程：在 image-only baseline (train_mimic_resnet18.py) 的基础上，
把分类头换成由离散敏感属性 (sex / race / age_group) 驱动的 HyperNetwork
(src/models/resnet18_hyperhead.py)。backbone 与 baseline 完全一致 (ImageNet 预训练
torchvision ResNet-18)，因此可与 baseline 在相同预处理、相同早停设置下直接对比公平性。

与 baseline 的唯一区别：图像之外，sex / race / age_group 会进入模型 (作为 HyperNet
的条件)，而非仅作为分组键。其余训练策略 (AdamW + 小学习率 + weight decay + 梯度裁剪、
两种 model selection、早停监控 val Overall AUC、超参均读 configs/chexpert_baseline.yaml)
与 baseline 保持一致。
...
运行方式:
    重型任务，必须通过 sbatch 提交到 SLURM 集群 (见 slurm/train_chexpert_hyperhead.sh)。
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Train ResNet18-HyperHead on CheXpert (No Finding, attribute-conditioned)"
    )
...
def train_one_epoch(
    ...
    for i, (images, labels, sexes, races, ages) in enumerate(loader):
        # 与 baseline 不同：敏感属性进入模型，作为 HyperNet 的条件
...
def evaluate(model: nn.Module, loader: DataLoader, criterion: nn.Module) -> tuple[float, float, float, float]:
    ...
    敏感属性既作为模型输入 (HyperNet 条件)，又作为公平性分组键。
    """
    model.eval()
    total_loss = 0.0
    correct = 0
...
def main() -> None:
    ...
    ckpt_overall_path = OUTPUT_DIR / f"hyperhead_chexpert_no_finding_seed{seed}_best_overall.pth"
    ckpt_worstcase_path = OUTPUT_DIR / f"hyperhead_chexpert_no_finding_seed{seed}_best_worstcase.pth"
    ...
    model = ResNet18HyperHead(num_classes=1).to(DEVICE)
    backbone_params = sum(p.numel() for p in model.backbone.parameters())
    hyper_params = sum(p.numel() for p in model.hyper.parameters())
    print(f"\nModel : ResNet18-HyperHead (backbone=ImageNet 预训练, head 由 sex/race/age 生成)")
    print(f"Params: {backbone_params + hyper_params:,} (backbone {backbone_params:,} + hyper {hyper_params:,})")
