"""
Train ResNet18-HyperAdapt on Fitzpatrick17k (malignant, skin-conditioned)
=========================================================================
把 backbone **每个卷积层**（通道级乘性调制）与分类头 fc（加性低秩更新）都接上由**单一敏感属性 skin**
（Fitzpatrick I–VI → 0–5）驱动的超网络（ResNet18HyperAdaptSkin，初始 Δθ≈0 起步等价 baseline）。
HyperAdapt 是「介入最遍布」的深层 HN（除 stem 外每层）。
...
运行方式：重型任务，经 sbatch 提交（见 slurm/train_fitzpatrick_hyperadapt.sh）。
"""
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(description="Train ResNet18-HyperAdapt on Fitzpatrick17k (skin-conditioned)")
