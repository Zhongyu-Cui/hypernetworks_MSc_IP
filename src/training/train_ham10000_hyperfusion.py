"""
Train ResNet18-HyperFusion on HAM10000 for Malignant Binary Classification (age-conditioned)
===========================================================================================
属性融合变体训练流程：在 image-only baseline (src/training/train_ham10000_resnet18.py) 的
基础上，仅在 backbone 的 **layer4[0].downsample 1x1 卷积** 处接一个由**单一敏感属性 age_group**
(20-40/40-60/60-80/80+ → 0–3) 驱动的 HyperFusion 超网络 (src/models/resnet18_hyperfusion.py 的
ResNet18HyperFusionAge)：超网络依据 age 逐样本生成该 downsample 卷积权重的残差扰动 θ_ds=θ_0+Δθ
(MIP additive + E_L2 投影)，其余所有层 (stem/layer1~3/layer4 主分支/layer4[1]/fc) 与 baseline 完全
一致、共享权重。MIP 初始化令初始 Δθ≈0、起步严格等价于纯预训练 baseline，可在相同预处理/早停下直接对比。

为什么 HyperFusion + 只条件化 age：HAM 条件互信息诊断 (作业 68297) 显示 sex 轴 I(Y;sex|X)≈0、
age 轴读出可复现 I(Y;age|X)>0，故先只接 age（sex 轴无信号不接）。HyperFusion 是「单点、深层、
全权重生成」的注入 (对齐 Ortiz et al. ICLR 2024)，与 HyperHead「只改 fc」、HyperAdapt「每个 conv
低秩调制」形成注入位置/深度/容量的三方对照。

与 HyperHead / HyperAdapt 训练脚本的差异：
  - 模型换成 ResNet18HyperFusionAge（注入点 = layer4[0].downsample，单点深层全权重生成）
  - 显存接近 baseline（仅单个 1x1 conv 逐样本，非每层逐样本卷积核），故用原 batch_size=128（无需调小）

与 baseline (image-only) 训练脚本的区别：
  - 模型换成 ResNet18HyperFusionAge，训练/评估把 age 喂进模型 model(images, age)
...
运行方式:
    重型任务，必须通过 sbatch 提交到 SLURM 集群 (见 slurm/train_ham10000_hyperfusion.sh)。
...
def parse_args() -> argparse.Namespace:
    """解析命令行参数（--seed 多 seed 重复实验；--batch_size 可选覆盖，HyperFusion 通常无需）。"""
    parser = argparse.ArgumentParser(
        description="Train ResNet18-HyperFusion on HAM10000 (malignant, age-conditioned)"
    )
    parser.add_argument("--seed", type=int, default=None,
                        help="随机种子；缺省时取 configs/ham10000_baseline.yaml 中 training.seeds[0]")
    parser.add_argument("--batch_size", type=int, default=None,
                        help="覆盖 config 的 training.batch_size；HyperFusion 单点 1x1 conv 注入、显存接近 baseline，"
...
def train_one_epoch(
    ...
    for i, (images, labels, _sex, ages) in enumerate(loader):
        # 与 baseline 不同：age 进入模型，作为 HyperFusion 超网络的条件（sex 仍不进模型）
...
def main() -> None:
    ...
    # CLI 可选覆盖 batch_size（HyperFusion 通常无需；保留以便统一接口）
    ...
    ckpt_overall_path = OUTPUT_DIR / f"hyperfusion_age_malignant_seed{seed}_best_overall.pth"
    ckpt_worstcase_path = OUTPUT_DIR / f"hyperfusion_age_malignant_seed{seed}_best_worstcase.pth"

    print("\nLoading HAM10000 (malignant, age-conditioned HyperFusion)...")
    ...
    # backbone 与 baseline 一致 (ImageNet 预训练)；HyperFusion MIP 初始 Δθ≈0、起步等价 baseline
    model = ResNet18HyperFusionAge(num_classes=1, num_age=4,
                                   pretrained=cfg["model"]["pretrained"]).to(DEVICE)
    n_total, n_hyper, n_backbone = model.param_breakdown()
    print(f"\nModel : ResNet18-HyperFusion (backbone=ImageNet 预训练共享, layer4[0].downsample 由 age 逐样本生成)")
    print(f"Params: {n_total:,} (backbone {n_backbone:,} + hyper {n_hyper:,})")
