"""
Train ResNet18-HyperHead on MIMIC-CXR for No Finding Binary Classification
==========================================================================
...
两种 model selection、早停监控 val Overall AUC、超参均读 configs/mimic_cxr_baseline.yaml)
与 baseline 保持一致。
...
运行方式:
    重型任务，必须通过 sbatch 提交到 SLURM 集群 (见 slurm/train_mimic_hyperhead.sh)。
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Train ResNet18-HyperHead on MIMIC-CXR (No Finding, attribute-conditioned)"
    )
...
def main() -> None:
    ...
    ckpt_overall_path = OUTPUT_DIR / f"hyperhead_no_finding_seed{seed}_best_overall.pth"
    ckpt_worstcase_path = OUTPUT_DIR / f"hyperhead_no_finding_seed{seed}_best_worstcase.pth"
