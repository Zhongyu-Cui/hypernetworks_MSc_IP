"""
Train ResNet18 on CheXpert for No Finding Binary Classification (Image-only)
=============================================================================
端到端 baseline 训练流程：仅以胸部 X 光图像作为输入（不引入 sex / race / age），
预测 No Finding（官方全报告 CheXbert 标注：0=有病灶，1=无病灶/健康）。

与 MIMIC baseline（train_mimic_resnet18.py）**严格同参同结构**，仅换数据源与输出路径，
以保证 MIMIC↔CheXpert OOD 对照在同一套加载/transform/评估口径下可比。
Dataset 直接复用 MIMICCXRDataset（CheXpert 与 MIMIC 在 master CSV 中 schema 同构、
PNG 同为 16-bit）。

⚠️ 标签来源（2026-06-29 修正）：旧 baseline 用 master 的 findings 段标签（75.7% 健康，
为空 findings 段伪影），已弃用、相关 checkpoint 移至 deprecated_findings_label/。现用官方
全报告标注（report_fixed.json，正例率 ~5.8%，少数类=健康），与 MIMIC 全报告口径一致。
baseline 仍用裸 BCEWithLogitsLoss + 不重采样（与 MIMIC 对齐）。⚠️ 正例率仅 5.8%：
logits>0 阈值下几乎全判 label=0，acc/FPR/FNR 无意义，AUC（阈值无关）为唯一主指标。
...
运行方式:
    重型任务，必须通过 sbatch 提交到 SLURM 集群 (见 slurm/train_chexpert_resnet18.sh)。
...
def parse_args() -> argparse.Namespace:
    """解析命令行参数（--seed 多 seed 重复实验；--resample_alpha 可选重采样）。"""
    parser = argparse.ArgumentParser(description="Train ResNet18 on CheXpert (No Finding, image-only)")
...
def get_dataloaders(
    ...
    三个 split 已由 scripts/build_chexpert_splits_nofinding.py 按患者级划分写入独立 CSV
    (data/splits/chexpert_nofinding/{train,val,test}.csv)，此处直接加载。
...
def main() -> None:
    ...
    ckpt_overall_path = OUTPUT_DIR / f"resnet18_chexpert_no_finding{tag}_seed{seed}_best_overall.pth"
    ckpt_worstcase_path = OUTPUT_DIR / f"resnet18_chexpert_no_finding{tag}_seed{seed}_best_worstcase.pth"

    print("\nLoading CheXpert (No Finding, U-Zeros)...")
