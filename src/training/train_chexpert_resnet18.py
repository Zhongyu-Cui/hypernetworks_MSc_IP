"""
Train ResNet18 on CheXpert for No Finding Binary Classification (Image-only, ERM)
================================================================================
CheXpert-Plus No Finding 二分类（0=有病灶, 1=无病灶/健康）。字段与 MIMIC-CXR 同构，复用 MIMIC 的
Dataset / 公平性模块。⚠️ 正例率仅 ~5.8%（极不均衡）：acc/FPR/FNR 无意义，**AUC 为唯一主指标**；
CXR7-1M PNG 为 16-bit，Dataset 内部已做 min-max 归一化。

**比较协议改造（A0.4）**：训练循环统一走 harness.run_training；本脚本保留 CXR transforms / dataloader，
注入 unpack_sex_race_age + forward_image_only + print_mimic_fairness_report。超参 lr/wd 由
--config_index 从网格取（缺省 index=2=中心点）。

运行方式：重型任务，经 sbatch 提交（见 slurm/train_chexpert_resnet18.sh）。
"""
...
def parse_args() -> argparse.Namespace:
    """解析命令行参数（--config_index 超参网格；--seed；--resample_alpha 平衡重采样）。"""
    parser = argparse.ArgumentParser(description="Train ResNet18 on CheXpert (No Finding, image-only, ERM)")
