"""
Train ResNet18-HyperFusion on CheXpert for No Finding Binary Classification
============================================================================
属性融合变体训练流程：在 image-only baseline (train_mimic_resnet18.py) 的基础上，
仅在 backbone 的 layer4[0].downsample 1x1 卷积处接由离散敏感属性 (sex / race /
age_group) 驱动的 HyperFusion 超网络 (src/models/resnet18_hyperfusion.py)。backbone
与 baseline 完全一致 (ImageNet 预训练 torchvision ResNet-18)，MIP 初始化保证初始
Δθ≈0、起始等价于纯预训练 backbone，因此可与 baseline 在相同预处理、相同早停设置下
直接对比公平性。

与 HyperAdapt 训练脚本的唯一区别：模型换成 ResNet18HyperFusion（注入更集中——只在
高层 shortcut 单点全权重生成，而非每个卷积层低秩调制）。其余训练策略 (AdamW + 小学习率
+ weight decay + 梯度裁剪、两种 model selection、早停监控 val Overall AUC、超参均读
configs/chexpert_baseline.yaml) 与 baseline 严格保持一致。

显存：HyperFusion 仅在单个 1x1 downsample 卷积上做 grouped-conv 逐样本展开，显存占用
接近 baseline / HyperHead（远低于每层都展开的 HyperAdapt），batch_size=128 在 gpus24
(24GiB) 上即可。
...
运行方式:
    重型任务，必须通过 sbatch 提交到 SLURM 集群 (见 slurm/train_chexpert_hyperfusion.sh)。
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Train ResNet18-HyperFusion on CheXpert (No Finding, attribute-conditioned)"
    )
...
def train_one_epoch(
    ...
    for i, (images, labels, sexes, races, ages) in enumerate(loader):
        # 与 baseline 不同：敏感属性进入模型，作为 HyperFusion 超网络的条件
...
def main() -> None:
    ...
    ckpt_overall_path = OUTPUT_DIR / f"hyperfusion_chexpert_no_finding_seed{seed}_best_overall.pth"
    ckpt_worstcase_path = OUTPUT_DIR / f"hyperfusion_chexpert_no_finding_seed{seed}_best_worstcase.pth"
    ...
    # pretrained=True：backbone 与 baseline 一致 (ImageNet 预训练)，MIP 初始 Δθ≈0
    model = ResNet18HyperFusion(num_classes=1, pretrained=cfg["model"]["pretrained"]).to(DEVICE)
    n_total, n_hyper, n_backbone = model.param_breakdown()
    print(f"\nModel : ResNet18-HyperFusion (backbone=ImageNet 预训练, layer4[0].downsample 由 sex/race/age 生成)")
    print(f"Params: {n_total:,} (backbone {n_backbone:,} + hypernet {n_hyper:,})")
