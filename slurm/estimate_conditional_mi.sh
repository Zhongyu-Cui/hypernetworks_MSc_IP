...
#SBATCH --gres=gpu:1
#SBATCH --output=/vol/biomedic2/bglocker_studproj/zc125/logs/estimate_conditional_mi.%N.%j.log
#SBATCH --time=0-03:59:00
#SBATCH --job-name=cmi_diag

# 估计 MIMIC-CXR I(Y;A|X)，检验敏感属性信号是否弱（E0–E3）。
# 特征抽取（~199k 张图各一次前向）需 GPU；E0–E3 分析随后在缓存特征上跑（同作业）。
...
python scripts/estimate_conditional_mi.py
